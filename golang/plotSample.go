...
func createPlots(allSignal map[int]map[int][][]int, 
	...
	if flanksInvalidSize == 0 {
		figDat, hp1, hp2 = figData(allSignal, flanks, start, stop, svLen)
	} else {
		figDat, hp1, hp2 = figData(allSignal, flanks, start, stop, svLen)
		for _, flank := range flanksInvalid {
			flankData := make(map[int][][][]int)
			flankData[6] = [][][]int{{{start, 1}, {start, 25}, {-1}}, {{stop, 1}, {stop, 25},{-1}}}
        ...
        colors := map[string][]color.RGBA{
                  "DEL":{
                          ...
                          {R: 0, G: 0, B: 0, A: 255},     //ref record 7
                        },
                  "INS":{
                          {R: 1, G: 193, B: 110, A: 255},   //1 L
                          {R: 3, G: 126, B: 243, A: 255},   //2 Del
	...
	// 正向循环遍历colorLinesMap，从最小的flank开始绘制

        colorIds := []int{5, 6, 1, 4, 3, 2,7}
        switch svtype{
        case "DEL":
            colorIds = []int{5, 6, 2, 7}
        case "INS":
            colorIds = []int{5, 6, 3, 7}
        case "DUP":
            colorIds = []int{5, 6, 1, 4, 3, 2, 7}
        case "INV":
            colorIds = []int{5, 6, 1, 4, 3, 2, 7}
			...
			for _, line := range lines {
				...
				if err != nil {
				    ...
				    line.Color = colors[svtype][colorId-1]
                                    ...
                                    fmt.Println("alignColors:", int(int(math.Abs(float64(mapQ-1))) / 10), mapQ, int(int(math.Abs(float64(mapQ-1))) / 10))
...
func figData(allSignal map[int]map[int][][]int, flanks []int,
             start int, stop int, svLen int) (map[int]map[int][][][]int, int, int) {
	figDat := make(map[int]map[int][][][]int)
	// 外部转换和排序
	...
	for flank_i, flank := range flanks {
		...
		flankData[6] = [][][]int{{{start, 1}, {start, nu}, {-1}}, {{stop, 1}, {stop, nu}, {-1}}}
                if start+svLen > stop + flank{
                    flankData[7] = [][][]int{{{start, nu}, {stop + flank, nu}, {-1}}} //ref size
                }else{
                    flankData[7] = [][][]int{{{start, nu}, {start+svLen, nu}, {-1}}} //ref size
                }
...
// splitString 将输入字符串拆分为数字和字母两个数组
...
func cutRead(query_start int, seq string, quality string, sig_len int, seq_len int) (string, string, int, int){
    tmp_s := query_start-sig_len
    ...
    return seq[tmp_s:tmp_e], quality[tmp_s:tmp_e], query_start-sig_len, tmp_e
}

func DealPosition(numbers []int, letters []string, query_name string, query_seq string, query_quality string, genome *fai.File,  chromosomeMap map[string]int, chrom string, alStart int, minSignal int) ([][]int) {
	refs_start := alStart
        query_start := 0
	signals := make([][]int, 0)
        seq_len := len([]rune(query_seq))
	...
	if strings.EqualFold(letters[0], "S") { //1
                ...
                if numbers[0] >= minSignal{
                    ...
                    if tmp_start > 0 {
                        newArray := []int{tmp_start, refs_start, 1}
                        signals = append(signals, newArray)
                        //seq_s1, quality_s1, _, _ := cutRead(query_start, query_seq, query_quality, 2000,  seq_len)
                        seq_s1, _, _, _ := cutRead(query_start, query_seq, query_quality, 2000,  seq_len)
                        //genomeSeq := faidx(genome, chrom, refs_start-3000, refs_start+2000, chromosomeMap[chrom])
                        fmt.Println(refs_start,"###S1:",seq_s1)
                        //if !strings.Contains(seq_s1, "N") && !strings.Contains(genomeSeq, "N") {
	                //        align1, align2, score, identity := globalAlignment(seq_s1, genomeSeq)
        	        //        fmt.Println("###S1:", len(quality_s1),len(align1), len(align2), chrom, tmp_start, refs_start, score, identity)
                	//        fmt.Println("###S11:", align1)
                        //	fmt.Println("###S12:", align2)
			//}
                    }
                }
	}
	for i := 0; i < len(letters); i++ {
		...
		if strings.EqualFold(letters[i], "M") {
			...
			if length >= minSignal {
				newArray := []int{refs_start, refs_stop, 2}
				signals = append(signals, newArray)
                                //seq_d, quality_d, tmp_s, tmp_e := cutRead(query_start, query_seq, query_quality, 350, seq_len)
                                seq_d, _, _,  _ := cutRead(query_start, query_seq, query_quality, 2000, seq_len)
                                //genomeSeq := faidx(genome, chrom, refs_start-300, refs_start+300, chromosomeMap[chrom])
                                fmt.Println(refs_start, refs_stop, "###D1:",seq_d)
                                //if !strings.Contains(seq_d, "N") && !strings.Contains(genomeSeq, "N") {
                                //	align1, align2, score, identity := globalAlignment(seq_d, genomeSeq)
                                //        fmt.Println("###D:", len(quality_d),len(align1), len(align2), chrom, refs_start, refs_stop, score, identity, "###",tmp_s, tmp_e, 350, query_start, seq_len)
        	                //        fmt.Println("###D_read:", align1)
	                        //        fmt.Println("###D_ref:", align2)
                                // }
			}
			...
			if length >= minSignal {
				newArray := []int{refs_start, reads_stop, 3}
				signals = append(signals, newArray) 
                                //seq_i, quality_i, _, _ := cutRead(query_start, query_seq, query_quality, 300, seq_len)
                                seq_i, _, _, _ := cutRead(query_start, query_seq, query_quality, 2000, seq_len)
                                //genomeSeq := faidx(genome, chrom, refs_start-300, refs_start+300, chromosomeMap[chrom])
                                fmt.Println(refs_start, reads_stop, "###I1:",seq_i)
                                //if !strings.Contains(seq_i, "N") && !strings.Contains(genomeSeq, "N") {
	                        //        align1, align2, score, identity := globalAlignment(seq_i, genomeSeq)
        	                //        fmt.Println("###I:", len(quality_i),len(align1), len(align2), chrom, refs_start, reads_stop, score, identity)
                	        //        fmt.Println("###I:", align1)
                        	//        fmt.Println("###I:", align2)
                                //}
			}
                        query_start = query_start + length
	...
	if strings.EqualFold(letters[len(letters)-1], "S") { //4
		newArray := []int{refs_start, refs_start + numbers[len(letters)-1], 4}
		signals = append(signals, newArray)
                //seq_s2, quality_s2, _, _ := cutRead(query_start, query_seq, query_quality, 300, seq_len)
                seq_s2, _,  _, _ := cutRead(query_start, query_seq, query_quality, 2000, seq_len)
                //fmt.Println("faidx:", genome, chrom, refs_start-300, refs_start+300, chromosomeMap[chrom])
                //genomeSeq := faidx(genome, chrom, refs_start-300, refs_start+300, chromosomeMap[chrom])
                fmt.Println(refs_start,"###S2:",seq_s2)
                //if !strings.Contains(seq_s2, "N") && !strings.Contains(genomeSeq, "N") {
	        //         align1, align2, score, identity := globalAlignment(seq_s2, genomeSeq)
        	//        fmt.Println("###S2:", len(quality_s2),len(align1), len(align2), chrom, refs_start, refs_start + numbers[len(letters)-1], score, identity)
                //	fmt.Println("###S21:", align1)
	        //        fmt.Println("###S22:", align2)
		//}
	}
...
func KeyInMap(allSignals *map[int]map[int][][]int, flank int, nums int, tmp []int) {
        allSignalsMap := *allSignals
	if _, ok1 := allSignalsMap[flank]; ok1 {
		if _, ok2 := allSignalsMap[flank][nums]; ok2 {
			allSignalsMap[flank][nums] = append(allSignalsMap[flank][nums], tmp)
		} else {
			allSignalsMap[flank][nums] = [][]int{tmp}
		}
	} else {
		allSignalsMap[flank] = map[int][][]int{nums: {tmp}}
	}
}
...
func CutFlank(tmp []int, minPos int, maxPos int, flank int, nums int) ([]int, bool) {
        x1 := tmp[0]
        x2 := tmp[1]
        flag := tmp[2]
        tmp1 := make([]int, 5)
...
func CutFlankDeal(start int, stop int,
                  ailgnStart int, alignStop int, 
                  flank int, nums int, 
                  map_quality int, signals [][]int, 
                  allSignals *map[int]map[int][][]int) {
	...
	for i := 0; i < len(signals); i++ {
		tmp, flg := CutFlank(signals[i], fStart, fStop, flank,  nums)
		if flg {
		     KeyInMap(allSignals, flank, nums, tmp)
		}
	}
	ailgnPosition := []int{ailgnStart, alignStop, map_quality}
	tmp1, flg := CutFlank(ailgnPosition, fStart, fStop, flank, nums)
	if flg {
		KeyInMap(allSignals, flank, nums, tmp1)
	}
}


func dealReads(input string, flanks []int, 
               query_name string, query_seq string, query_quality string,
               genome *fai.File, chromosomeMap map[string]int, chrom string,
               start int, stop int, 
               minSignal int, alStart int, 
               alStop int, num int, 
               map_quality int, flanksInvalid *map[int]int, 
               allSignals *map[int]map[int][][]int){

	// 将结果发送到通道
	var signals [][]int
	numbers, letters := SplitString(input)
        //DealPosition(numbers []int, letters []string, query_name string, query_seq string, query_quality string, genome *fai.File,  chromosomeMap map[string]int, chrom string, alStart int, minSignal int) 
	//signals  = DealPosition(numbers, letters, alStart, minSignal)
        signals  = DealPosition(numbers, letters, query_name, query_seq, query_quality, genome, chromosomeMap, chrom, alStart, minSignal)
        allSignalsMap := *allSignals
        flanksInvalidMap := *flanksInvalid
        rawSiz := len(allSignalsMap)
	for _, flank := range flanks {
                ...
                newSize := len(allSignalsMap)
		if newSize > rawSiz {
			delete(flanksInvalidMap, flank)
		}
                rawSiz = newSize
	}
}
...
func stringtomap(str_value string, str_keys []string) map[string][]string {
	str_values := strings.Split(str_value, ",")
	data := make(map[string][]string)
	for key, value := range str_values {
		values := strings.Split(value, ":")
		data[str_keys[key]] = values
	}
	return data
}

func caculationTime(startTime time.Time, str string){
    // 记录函数结束时间
    endTime := time.Now()
...
func genomeLength(genomeFile string)(map[string]int){
    faiFilePath := genomeFile+".fai"

    faiFile, err := os.Open(faiFilePath)
        if err != nil {
                fmt.Println("Error opening file:", err)
        }
        defer faiFile.Close()

    chromosomeMap := make(map[string]int)
    ...
    for scanner.Scan() {
        ...
        if len(fields) >= 2 {
            chromosomeID := fields[0]
            chromosomeLength := atoi(fields[1])

            chromosomeMap[chromosomeID] = chromosomeLength
        }
    }
...
func faidx(m *fai.File, chrom string, start int, stop int, chrom_stop int)(string){
        if stop > chrom_stop{
            stop = chrom_stop
        }
...
// FastaSequence 表示FASTA格式的序列
type FastaSequence struct {
        Header string
        Sequence string
}

// ReadFasta 从FASTA文件中读取序列
func ReadFasta(filename string) ([]FastaSequence, error) {
        ...
        var currentSequence FastaSequence
        ...
        for scanner.Scan() {
                ...
                if strings.HasPrefix(line, ">") {
                        ...
                        currentSequence = FastaSequence{Header: line[1:], Sequence: ""}
                } else {
                        // 序列行
...
// WriteFasta 将FASTA序列写入文件
func WriteFasta(filename string, readSeq string, refSeq string) error {
	read := FastaSequence{Header: "read", Sequence: readSeq}
	ref := FastaSequence{Header: "ref", Sequence: refSeq}
	
	sequences := []FastaSequence{read, ref}
	...
	for _, seq := range sequences {
		_, err := writer.WriteString(">" + seq.Header + "\n" + seq.Sequence + "\n")
		if err != nil {
			return err
		}
	}
	...
	//fmt.Printf("FASTA文件 '%s' 写入成功\n", filename)
	return nil
}

func reAlignSigs(input string) [][]int {
        pattern := `-+`
        ...
        for _, match := range matches {
                ...
                start := match[0]
                if start == 0{
                    result = append(result, []int{length, start})
                }else{
                    result = append(result, []int{length, start-tmp})
                }
                tmp += length
        }
...
func runKalign(read, ref string)(string, string){
    cmd := exec.Command("bash", "-c", fmt.Sprintf("echo -e \">read\n%s\n>ref\n%s\"|kalign --type dna", read, ref))
    output, err := cmd.Output()   // 调用 Output() 函数来执行命令并返回输出结果
    if err != nil {                // 处理错误情况
        log.Fatal(err)
    }

    result := strings.ReplaceAll(string(output), "\n", "")
    regex1 := regexp.MustCompile(`.*?>read`)
    result2 := regex1.ReplaceAllString(result, "")
    read_ref := strings.Split(result2, ">ref")

    //sigs := reAlignSigs(read_ref[0])   
    return read_ref[0], read_ref[1]
}


func globalAlignment(read, ref string)(string, string, int, float32) { //NWAffine_Align
    cmd := exec.Command("bash", "-c", fmt.Sprintf("echo -e \">read\n%s\n>ref\n%s\"|kalign --type dna", read, ref))
    ...
    return read_ref[0], read_ref[1], 100, 0.98
}
...
//export DealWith
func DealWith(c_cigas *C.char,
              c_query_names *C.char,
              c_query_seqs *C.char, 
              c_query_quality *C.char,
              c_chrom *C.char, 
              c_svtype *C.char, 
              Rflanks *C.char, 
              c_genome *C.char,
    ...
    genomeFile := C.GoString(c_genome)
    genome, fasta := genomeIndex(genomeFile)
    chromosomeMap := genomeLength(genomeFile)

    query_seqs := strings.Split(C.GoString(c_query_seqs), ",")
    query_quality := strings.Split(C.GoString(c_query_quality), " ")
    ...
    cigasmap := stringtomap(C.GoString(c_cigas), samples)
    alignStartmap := stringtomap(C.GoString(c_alignStart), samples)
    alignStopmap := stringtomap(C.GoString(c_alignStop), samples)
    numsmap := stringtomap(C.GoString(c_nums), samples)
    map_quality_map := stringtomap(C.GoString(c_map_quality), samples)
    ...
    flanksInvalid_tmp := map[int]int{}
    for i, str := range rfls {
        num, _ := strconv.Atoi(str)
        flanks[i] = num
        flanksInvalid_tmp[num] = num
    }
    sort.Ints(flanks)

    caculationTime(startTime, "a2")
    startTime = time.Now() //
    samplesplots := make(map[string][]*plot.Plot)
    samplePlotGT  := make(map[string]*plot.Plot)

    
    for sample, cigas := range cigasmap {
        //alignStart := alignStartmap[sample]
        //alignStop := alignStopmap[sample]
        //nums := numsmap[sample]
        //map_qualitys  := map_quality_map[sample]

        flanksInvalid := flanksInvalid_tmp
        allSignals := make(map[int]map[int][][]int)

	for i, input := range cigas{
	    //alStart_s, _ := strconv.Atoi(alignStart[i])
	    //alStop_s, _ := strconv.Atoi(alignStop[i])
	    //num_s, _ := strconv.Atoi(nums[i])
            //map_quality_s, _ := strconv.Atoi(map_qualitys[i])

            alStart_s, _ := strconv.Atoi(alignStartmap[sample][i])
            alStop_s, _ := strconv.Atoi(alignStopmap[sample][i])
            num_s, _ := strconv.Atoi(numsmap[sample][i])
            map_quality_s, _ := strconv.Atoi(map_quality_map[sample][i])
            
            //fmt.Println("map_quality_s:",map_quality_s, map_quality_map[sample][i], input)
            dealReads(
                     input,
                     flanks,
                     query_names[i], 
        ...
        caculationTime(startTime, "a3")
        startTime = time.Now() //
	//fmt.Println(builder.String())
	//fmt.Println(sample,sample,builder.String())
	sampleplots, pBar := createPlots(
        ...
        //sampleplots = append(sampleplots, pBar)
        samplePlotGT[sample] = pBar
	samplesplots[sample] = sampleplots
        caculationTime(startTime, "a4")
        startTime = time.Now() //
    }
    caculationTime(startTime, "a5")
    startTime = time.Now() //
    ...
    xlabel := chrom //+ "_" + svtype
    flanksID := flanktoids(flanks)
    //infos := "CHROM: " + chrom + "\nSVLEN: " + strconv.Itoa(svlen) + "\nPOS: " + strconv.Itoa(start) + "\nEND: " + strconv.Itoa(stop) + "\nSVTYPE: " + svtype + "\n"
    CombineAndSave(
                   samplesplots,  
                   samplePlotGT,
                   samplePlotGene,
